...
public class MqClassLoader extends ClassLoader {
     ...
     * 强制载入一批类
     *
     * @param wildnames 可以是包名，也可以是类名的数组
     */
    ...
    public static void invoke(String method, Object[] obj, Class<?>[] parameterTypes) {
        try {
            Object cls = MqClassLoader.instance.loadClass("类全限定名", true).newInstance();
            cls.getClass().getMethod(method, parameterTypes).invoke(cls, obj);
        } catch (Exception e) {
            System.out.println("reloadable error " + method);
        }
    }
