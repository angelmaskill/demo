...
public class NettyServerBootstrap {
	...
	private void bind() {
		 ...
		 * 
		 * For instance, imagine you have a queue of tasks on server side that is filled by clients and processed by backend. In case clients send tasks too quick the length of the queue grows. 
		 * One needs to introduce so named high watermark and low watermark. If queue length is greater than high watermark stop reading from sockets and queue length will decrease. 
		 * When queue length becomes less than low watermark start reading tasks from sockets again.
		 * Note, to make it possible for clients to adapt to speed you process tasks (actually to adapt window size) one shouldn't make a big gap between high and low watermarks. 
